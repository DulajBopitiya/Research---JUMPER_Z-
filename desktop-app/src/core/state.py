...
class AppState:
    ...
    def set(self, key: StateKeys, value: Any, *, owner: str) -> None:
        ...
        if expected_owner and expected_owner != owner:
            logger.error(
                "State ownership violation: %s tried to set %s owned by %s",
                owner,
                key,
                expected_owner,
        ...
        try:
            ...
            logger.exception(
                "AppState: error emitting set for %s",
                key,
    ...
    def delete(self, key: StateKeys, *, owner: str) -> None:
        ...
        if expected_owner and expected_owner != owner and owner != "AppController":
            logger.error(
                "State deletion violation: %s tried to delete %s (owner=%s)",
                owner,
                key,
                expected_owner,
                extra={"event_id": L.CORE.STATE_VIOLATION},
        ...
        with self._lock:
            ...
            if existed:
                del self._data[key]
                logger.debug(
                    "AppState.delete: removed %s", key, extra={"event_id": L.CORE.STATE_CHANGED}
                )
        ...
        if existed:
            try:
                ...
                logger.exception(
                    "AppState: error emitting delete for %s",
                    key,
                    extra={"event_id": L.CORE.EVENT_HANDLER_ERROR},
                )
    ...
    def clear(self, *, owner: str) -> None:
        if owner != "AppController":
            logger.error(
                "State clear violation: only AppController can clear state (attempt by %s)",
                owner,
                extra={"event_id": L.CORE.STATE_VIOLATION},
            )
            return
        ...
        try:
            ...
            logger.exception(
                "AppState: error emitting clear", extra={"event_id": L.CORE.EVENT_HANDLER_ERROR}
            )

    def all(self) -> dict[StateKeys, Any]:
        with self._lock:
